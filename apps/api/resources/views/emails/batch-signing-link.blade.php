...
<head>
  ...
  <title>Documentos para Assinatura</title>
  <style>
    ...
    .document-list { background: #F8F7F4; border-left: 4px solid #C9A84C; padding: 16px 20px; border-radius: 4px; margin: 24px 0; }
    .document-list strong { display: block; font-size: 16px; color: #1B2E4B; margin-bottom: 8px; }
    .document-list ul { margin: 0; padding-left: 20px; }
    .document-list li { color: #374151; font-size: 14px; margin-bottom: 6px; }
    .badge { display: inline-block; background: #C9A84C; color: #1B2E4B; font-size: 12px; font-weight: 700; padding: 3px 10px; border-radius: 12px; margin-left: 4px; }
...
<body>
  <div class="container">
    <div class="header">
      <h1>📄 {{ $documentCount }} Documentos para Assinatura</h1>
      <p>Escritório {{ $lawyerName }}</p>
    </div>
    <div class="body">
      <p>Olá, <strong>{{ $clientName }}</strong>!</p>
      <p>O(A) advogado(a) <strong>{{ $lawyerName }}</strong>{{ $oabNumber ? ' (OAB ' . $oabNumber . ')' : '' }} enviou <strong>{{ $documentCount }} documentos</strong> para você assinar eletronicamente de uma só vez.</p>

      <div class="document-list">
        <strong>Documentos <span class="badge">{{ $documentCount }}</span></strong>
        <ul>
      ...
      <p style="font-size:14px;color:#374151;">
        Você poderá visualizar cada documento e assinar todos de uma vez, com uma única selfie e assinatura.
      </p>

      <p style="text-align:center;">
        <a href="{{ $signingUrl }}" class="btn">Visualizar e Assinar Documentos</a>
      </p>
    ...
    <div class="footer">
      <p>Este e-mail foi enviado porque você tem documentos jurídicos pendentes de assinatura.</p>
